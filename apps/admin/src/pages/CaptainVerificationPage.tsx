...
import {
  ...
  BadgeCheck, Fingerprint, CalendarClock, Settings2, Plus, Trash2
} from 'lucide-react';
...
interface Doc {
  ...
  type_title_ar?: string;
  type_title_en?: string;
  // Identity metadata captured at upload time
  // (migration 0012_document_identity_fields).
...
/* ------------------------------------------------------------------ */
/*  Document type catalog manager                                      */
/*                                                                     */
/*  Adding a new required document used to mean shipping new builds    */
/*  of the captain app + admin dashboard. Now the catalog lives in the */
/*  database and the captain app's upload grid renders whatever is     */
/*  active here — a new document (تأمين السيارة, شهادة صحية…) appears  */
/*  for captains the moment it is saved.                               */
/* ------------------------------------------------------------------ */
function DocTypesManagerModal({
  token,
  onClose,
  onChanged,
}: {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await api<{ types: DocType[] }>('/admin/document-types', { token });
      setTypes(res.types || []);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'فشل تحميل أنواع المستندات');
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  const addType = async () => {
    if (!newId.trim() || !newTitleAr.trim()) return;
    setSaving(true);
    setError(null);
    try {
      ...
      setError(e instanceof Error ? e.message : 'فشل إضافة النوع');
    } finally {
      setSaving(false);
    }
  };
  ...
  const toggleActive = async (t: DocType) => {
    try {
      await api(`/admin/document-types/${t.id}`, {
        method: 'PUT',
        token,
      ...
      alert('فشل تحديث النوع');
    }
  };

  const toggleRequired = async (t: DocType) => {
    try {
      await api(`/admin/document-types/${t.id}`, {
        method: 'PUT',
        token,
        body: JSON.stringify({ required: !t.required }),
      });
      await load();
      onChanged();
    } catch {
  ...
  const deactivate = async (t: DocType) => {
    if (!confirm(`إخفاء "${t.title_ar}" من تطبيق الكابتن؟ سيبقى سجل المستندات السابقة محفوظاً.`)) return;
    try {
      await api(`/admin/document-types/${t.id}`, { method: 'DELETE', token });
      await load();
      onChanged();
    } catch {
      alert('فشل حذف النوع');
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div
        className="bg-surface-primary border border-border-primary rounded-2xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col"
        dir="rtl"
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-border-primary">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-base text-text-primary">أنواع المستندات المطلوبة</h3>
              <p className="text-xs text-text-tertiary">ما تضيفه هنا يظهر فوراً في تطبيق الكابتن</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl bg-surface-secondary text-text-secondary hover:bg-surface-tertiary transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          {error && (
            <div className="p-3 bg-error-main/10 border border-error-main/30 rounded-xl text-error-main text-xs font-bold flex items-center gap-2">
              <AlertCircle className="w-4 h-4 shrink-0" /> {error}
            </div>
          )}

          {/* Add new type */}
          <div className="bg-surface-secondary/50 border border-border-primary rounded-xl p-4 space-y-3">
            <p className="text-xs font-extrabold text-text-secondary flex items-center gap-1.5">
              <Plus className="w-3.5 h-3.5 text-primary-500" /> إضافة مستند جديد
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              <input
                value={newTitleAr}
                onChange={(e) => setNewTitleAr(e.target.value)}
                placeholder="الاسم بالعربية (مثال: تأمين السيارة)"
                className="px-3 py-2.5 bg-surface-primary border border-border-primary rounded-xl text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-primary-500"
              />
              <input
                value={newTitleEn}
              ...
              <input
                ...
                dir="ltr"
                className="px-3 py-2.5 bg-surface-primary border border-border-primary rounded-xl text-sm text-text-primary placeholder:text-text-tertiary font-mono focus:outline-none focus:border-primary-500"
              />
            </div>
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-xs font-bold text-text-secondary cursor-pointer select-none">
                ...
                />
                مستند إلزامي (أوقف التالي حتى يُرفع)
              </label>
              <button
                onClick={addType}
                disabled={saving || !newId.trim() || !newTitleAr.trim()}
                className="flex items-center gap-1.5 px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white text-xs font-extrabold rounded-xl shadow-xs transition-colors disabled:opacity-50"
              >
          ...
          {/* Existing types */}
          {loading ? (
            <div className="py-8 flex justify-center"><Loader2 className="w-6 h-6 text-primary-500 animate-spin" /></div>
          ) : (
                ...
                <div
                  ...
                  className={`flex items-center justify-between gap-3 p-3 rounded-xl border transition-colors ${
                    t.active
                      ...
                      : 'bg-surface-secondary/40 border-border-primary/50 opacity-60'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${t.active ? 'bg-primary-500/10 text-primary-500' : 'bg-surface-tertiary text-text-tertiary'}`}>
                      <FileText className="w-4 h-4" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-text-primary truncate">{t.title_ar}</p>
                      <p className="text-[10px] text-text-tertiary font-mono truncate" dir="ltr">{t.id}{t.title_en ? ` • ${t.title_en}` : ''}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      onClick={() => toggleRequired(t)}
                      className={`px-2 py-1 text-[10px] font-extrabold rounded-lg border transition-colors ${
                        t.required
                          ? 'bg-warning-light text-warning-main border-warning-main/30'
                          : 'bg-surface-secondary text-text-tertiary border-border-primary'
                      }`}
                      title="تبديل إلزامي / اختياري"
                    >
                      {t.required ? 'إلزامي' : 'اختياري'}
                    </button>
                    <button
                    ...
                    {t.active && (
                      <button
                        ...
                        className="p-1.5 text-error-main hover:bg-error-main/10 rounded-lg transition-colors"
                        title="حذف (إخفاء)"
                      >
...
function ZoomablePreviewModal({
  ...
  // Resolved from the dynamic document-type catalog by the parent page, so the
  // preview header shows an admin-defined title instead of a raw type id.
  docTypeLabel: (d: Doc) => string;
}) {
...
export default function CaptainVerificationPage() {
  ...
  // Catalog-driven labels: title_ar from document_types (joined server-side),
  // falling back to the pre-catalog hard-coded map for unknown legacy types.
  const [typeLabels, setTypeLabels] = useState<Record<string, string>>(fallbackDocTypeLabels);

  const docTypeLabel = useCallback(
    (d: { type: string; type_title_ar?: string }) =>
      d.type_title_ar || typeLabels[d.type] || fallbackDocTypeLabels[d.type] || d.type,
    [typeLabels],
  );
  ...
  const loadTypeLabels = useCallback(async () => {
    try {
      ...
      (res.types || []).forEach((t) => { map[t.id] = t.title_ar; });
      setTypeLabels({ ...fallbackDocTypeLabels, ...map });
    } catch {
      // Older backend without the catalog: the fallback map keeps labels intact.
    }
  }, [token]);

  useEffect(() => { loadTypeLabels(); }, [loadTypeLabels]);
  ...
  return (
    <div className="space-y-6 animate-fade-in" dir="rtl">
        ...
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-extrabold rounded-xl bg-surface-secondary border border-border-primary text-text-secondary hover:text-text-primary hover:border-primary-500/40 transition-all"
            >
      ...
      {showTypesManager && (
        <DocTypesManagerModal
          token={token}
          onClose={() => setShowTypesManager(false)}
          onChanged={loadTypeLabels}
        />
      )}
