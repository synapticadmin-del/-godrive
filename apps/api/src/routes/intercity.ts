...
//
// index.ts already answers 404 for /intercity and /intercity/* — registered ahead of the
...
const INTERCITY_ENABLED: boolean = false;

intercityRoutes.use("*", async (c, next) => {
  if (INTERCITY_ENABLED) return next();
  return c.json(
    {
      error: "Intercity travel is not available",
