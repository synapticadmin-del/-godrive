...
///
/// The result is faster, lighter, and visually consistent on every device —
/// no video decoder, no aspect-ratio mismatch, no blank gap.
class SplashScreen extends StatefulWidget {
  const SplashScreen({super.key, this.onCompleted});

  /// Invoked once the intro has run its course.
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  /// Pre-cache the splash brand image so the first frame is instant.
  Future<void> _precacheBrandImage() async {
    ...
    // Hold the splash for a deliberate brand beat — long enough to be read,
    // short enough not to feel like a wait. Then hand over to the app shell.
  ...
  Widget build(BuildContext context) {
    // The splash is pinned to the light palette regardless of device dark
    // mode — the brand image is authored against a light canvas.
...
/// The high-resolution splash brand image, rendered with a subtle entrance.
...
/// Fallback when the brand image asset is unavailable — a clean logo lockup.
...
/// Wordmark, tagline and progress — anchored below the brand image.
