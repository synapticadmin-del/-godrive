-- 0014: Dynamic captain document types
--
...
CREATE TABLE IF NOT EXISTS document_types (
  ...
  required INTEGER NOT NULL DEFAULT 1,-- 0 = optional (Ø§Ø®ØªÙŠØ§Ø±ÙŠ badge in the app)
...
INSERT OR IGNORE INTO document_types (id, title_ar, title_en, icon, required, sort_order, active) VALUES
  ('license',             'Ø±Ø®ØµØ© Ø§Ù„Ù‚ÙŠØ§Ø¯Ø©',                        'Driving licence',            'card_membership', 1, 10, 1),
  ('national_id',         'Ø§Ù„Ø¨Ø·Ø§Ù‚Ø© Ø§Ù„Ø´Ø®ØµÙŠØ©',                     'National ID',                 'badge',           1, 20, 1),
  ('criminal_record',     'ØµØ­ÙŠÙØ© Ø§Ù„Ø­Ø§Ù„Ø© Ø§Ù„Ø¬Ù†Ø§Ø¦ÙŠØ©',               'Criminal record',            'fact_check',      1, 30, 1),
  ('criminal_record_back','Ø§Ù„Ø¬Ø§Ù†Ø¨ Ø§Ù„Ø®Ù„ÙÙŠ Ù„ØµØ­ÙŠÙØ© Ø§Ù„Ø­Ø§Ù„Ø© Ø§Ù„Ø¬Ù†Ø§Ø¦ÙŠØ©', 'Criminal record (back side)','fact_check',      0, 40, 1),
  ('vehicle_reg',         'Ø±Ø®ØµØ© Ø§Ù„Ø³ÙŠØ§Ø±Ø©',                        'Vehicle registration',        'directions_car',  1, 50, 1),
  ('vehicle_reg_back',    'Ø§Ù„Ø¬Ø§Ù†Ø¨ Ø§Ù„Ø®Ù„ÙÙŠ Ù„Ù„Ø´Ù‡Ø§Ø¯Ø©',               'Vehicle certificate (back)',  'directions_car',  0, 60, 1),
  ('vehicle_photo',       'ØµÙˆØ±Ø© Ø§Ù„Ù…Ø±ÙƒØ¨Ø©',                        'Vehicle photo',               'directions_car',  0, 70, 1),
  ('profile_photo',       'ØµÙˆØ±Ø© Ø´Ø®ØµÙŠØ©',                          'Personal photo',              'account_circle',  1, 5,  1);
