...
-- The operator queue: open alerts, oldest first. 0003 does index both columns,
-- but only separately -- idx_sos_status on status (0003:174) and idx_sos_created
-- on created_at (0003:175). SQLite uses at most one index per table per query,
-- so either one forces a sort or a scan for `WHERE status = ? ORDER BY
-- created_at`. This is the composite that filter-and-order actually needs.
-- (The draft this was adopted from said created_at was indexed alone; corrected
-- here, verified against 0003. The index itself is unchanged and still right.)
